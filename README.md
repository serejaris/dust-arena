...
Top-down multiplayer arena shooter в духе de_dust из Counter-Strike — браузерный, без сборки, реалтайм на WebSocket. Brawl-Stars-подобная телефото-камера сверху, командный дезматч на быстрых раундах.
...
- **Медкиты**, спавн-протекция, короткие раунды (1 мин), стрик-баннеры (ON FIRE / RAMPAGE / GODLIKE), тонты, киллфид, скорборд.
- **Звук** — SFX и голос-аннонсер сгенерированы через ElevenLabs (`tools/gen-sfx.mjs`), с процедурным фолбэком.
...
Игра собрана вживую на стриме вайбкодингом — задачи ставились AI-агентам через промпты (оригинальные промпты — в `prompt.md`, дизайн-ревью — в `design-review.md`, планы фич — в `plans/`).
